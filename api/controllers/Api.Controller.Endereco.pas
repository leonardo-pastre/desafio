...
begin
  LCEP := FRequest.Params.Items['cep'];
  ...
  if not ViaCEP(LCEP, LDados) then
    if not ApiCEP(LCEP, LDados) then
      if not AwesomeApi(LCEP, LDados) then
      begin
  ...
  try
    LRetorno := TJSONObject.ParseJSONValue(TEncoding.UTF8.GetBytes(LDados), 0) as TJSONObject;
    FResponse.Send<TJSONObject>(LRetorno).Status(THTTPStatus.OK);
  except
    on E: Exception do
